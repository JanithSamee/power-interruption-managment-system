...
const updateDevice = async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(404).json({ msg: "not-found", err: true, data: {} });
    }
...
const updateFixedBreakdown = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await Device.findByIdAndUpdate(id, { isunderBreakdown: false },{new:true});
    if (!data) {
      return res.status(404).json({ msg: "not-found", err: true, data: {} });
    }
    res.json({ msg: "success", err: false, data: data });
...
const activateDevice = async (req, res) => {
  try {
    ...
    const data = await Device.findByIdAndUpdate(id, { isActive: activate },{new:true});
    if (!data) {
      return res.status(404).json({ msg: "not-found", err: true, data: {} });
    }
    res.json({ msg: "success", err: false, data: data });
...
const getDevicesByregion = async (req, res) => {
  try {
    ...
    const data = await Device.find({ region,isActive:true },{createdAt:0,updatedAt:0});
    if (!data) {
      return res.status(404).json({ msg: "not-found", err: true, data: {} });
    }
    res.json({ msg: "success", err: false, data: data });
...
const submitBreakDown = async (req, res) => {
  try {
    ...
    const data = await Device.findOneAndUpdate(
      { devID ,isActive:true},
      { lastBreakdown: new Date(), isunderBreakdown: true },{new:true}
    );
    if (!data) {
...
const submitHighConsumtions = async (req, res) => {
  try {
    ...
    if(!data){
      ...
      data.highConsumtions = { recordedTime: new Date(), count: parseInt(data.highConsumtions.count + 1) };
      await data.save();
      res.send("success");
    }
  } catch (error) {
